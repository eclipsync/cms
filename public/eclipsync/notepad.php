...
define('PASSWORD', '99d025c5c787d5a2c170f11a14c7fd4cfaa4fae9c2a990460b2acbf072bbaedeb95db8cd7a9cc064e62d718dfbdf5b2e2cefd609b5cb73cad384e49f9e3e203e');
...
<head>
	...
	<title>' . $appName . '</title>
	<link id="favicon" rel="shortcut icon" type="image/png" href="' . $faviconImage. '" />
...
<body class="login-container"' . $background . '>
	<form method="post" class="login-form">
		<div class="box-form">
			<h1>
				<a class="title-lights title-big" href="' . $infoURL[0] . '" target="' . $infoURL[1] . '" title="' . $anchorTitle . '" style="color:#444;text-decoration:none" tabindex="3">' . $appName . '</a>
			</h1>' . (isset($error) ? ' 
			<p style="color:#dd0000">' . $error . '</p>' : null) . '
			<input id="' .  $nodeString . '_password" class="linetext" name="' .  $nodeString . '_password" type="password" value="" placeholder="' . $placeholder . '" tabindex="1" />
...
if (isset($_GET['logout'])) {
	if ($_GET['logout'] == $_SESSION[$nodeString . '_token']) {
		unset($_SESSION[$nodeString . '_admin']);
		session_destroy();
	}
	redirect();
}
...
if (isset($_GET['path'])) {
	...
	$session_token = $_SESSION[$nodeString . '_token'] ?? null;
	...
	switch ($_POST['action']) {
		case 'open' :
			...
			if (isset($_POST['file']) && file_exists(MAIN_DIR . $_POST['file'])) {
				die(json_success('OK', ['data' => file_get_contents(MAIN_DIR . $_POST['file'])]));
			}
			break;
		case 'save' :
...
$_SESSION[$nodeString . '_token'] = bin2hex(random_bytes(32));
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <link id="favicon" rel="shortcut icon" type="image/png" href="<?php echo $faviconImage;?>" />
...
<?php endforeach; ?>
		...
		var editor,
			...
			token = "<?php echo $_SESSION[$nodeString . '_token'] ?>";
		...
		$(function() {
			editor = CodeMirror.fromTextArea($("#editor")[0], {
				...
				indentUnit: 4,
				indentWithTabs: true,
				lineWrapping: <?php echo WORD_WRAP ?>,
			...
			$("#files > div").on("load_node.jstree", function(a, b) {
				...
				state: {
					key: "<?php echo $nodeString; ?>"
				},
				plugins:["state", "sort"],
				core: {
					data: {
						url: function(node) {
							return node.id == "#" ? "<?php echo $_SERVER['SCRIPT_NAME'] ?>?path=" : "<?php echo $_SERVER['SCRIPT_NAME'] ?>?path=" + node.a_attr["data-dir"];
						}
					}
				},
				'sort': function(a, b) {
			...
			$("#files").on("dblclick", "a[data-file]", function(event) {
				...
				?>
				window.open("<?php echo $base_dir ?>" + $(this).attr("data-file"));
			});
...
<body>
	...
	<div class="heading-alert alert alert-warning">
		<i class="fa fa-info-circle"></i>
		<span class="ml-2">
			You are using <?php echo $appName; ?> with default password. Please click <a href="javascript:void(0);" class="change-password">here</a> to change password after installation.
		</span>
	</div>
	<?php endif; ?>
	<div class="container-fluid">
		<div class="row p-3">
			<div class="col-md-3">
				<h1>
					<a href="<?php echo $infoURL[0];?>" class="title-lights title-medium" target="<?php echo $infoURL[1];?>" title="<?php echo $appName; ?> <?php echo VERSION ?>"><?php echo $appName; ?></a>
				</h1>
			</div>
			<div class="col-md-9">
				...
				<div class="float-right">
					...
					<a href="javascript:void(0);" class="change-password btn btn-sm btn-primary">
						<i class="fas fa-key"></i>
					</a> &nbsp; <?php } ?>
					<a href="<?php echo $_SERVER['SCRIPT_NAME'] ?>?logout=<?php echo $_SESSION[$nodeString . '_token'] ?>" class="btn btn-sm btn-danger">
	...
	<form method="post">
		...
		<input name="token" type="hidden" value="<?php echo $_SESSION[$nodeString . '_token'] ?>">
	...
	<div class="modal fade" id="helpModal">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="row">
<?php foreach($keyboard_shortcuts as $value) :?>
						<div class="col-12 col-sm-6 mb-1">
							<div class="row">
								<div class="col-6 text-right">
									<kbd><?php echo implode('</kbd><kbd>', $value[1]) ?></kbd>
								</div>
								<div class="col-6"><?php echo $value[0] ?></div>
							</div>
						</div>
<?php endforeach; ?>
					</div>
